...
// Triangle rasterization adapted from Polymaps:  
...
void scanSpan(Edge _e0, Edge _e1, 
              int _min, int _max, 
              const ScanCallback& _s
              ) 
{
    ...
    if (_e0.x0 == _e1.x0 && _e0.y0 == _e1.y0) 
    {
        ...
        if ( _e0.x1 - _e1.dy / _e0.dy * _e0.dx < _e1.x0 ) { 
            std::swap(_e0, _e1); 
        }
    }
...
void scanTriangle(glm::dvec2& _a, glm::dvec2& _b, glm::dvec2& _c, 
                  int _min, int _max, 
                  const ScanCallback& _s) 
{
