                                               ...
                                               const optional<style::ImageContent>& content) 
{
    ...
    if (width <= 0 || height <= 0 || width > 1024 || height > 1024 || 
        ratio <= 0 || ratio > 10 || srcX < 0 || srcY < 0 || 
        srcX >= static_cast<int32_t>(image.size.width) || 
        srcY >= static_cast<int32_t>(image.size.height) ||
    ...
    const Size size( static_cast<uint32_t>(width), 
                     static_cast<uint32_t>(height));
    PremultipliedImage dstImage(size);

    // Copy from the source image into our individual sprite image
    PremultipliedImage::copy(image, dstImage, 
                            {static_cast<uint32_t>(srcX), 
                             static_cast<uint32_t>(srcY)}, 
                             {0, 0}, size
                             );

    try {
        return std::make_unique<style::Image>(
            id, std::move(dstImage), ratio, sdf, 
...
uint16_t getUInt16(const JSValue& value, 
                   const char* property, 
                   const char* name, 
                   const uint16_t def = 0) 
{
    if (value.HasMember(property)) 
    {
        auto& v = value[property];
        if (v.IsUint() && 
            v.GetUint() <= std::numeric_limits<uint16_t>::max()) 
        {
            return v.GetUint();
        } else {
...
double getDouble(const JSValue& value, 
                 const char* property, 
                 const char* name, 
                 const double def = 0) 
{
    if (value.HasMember(property)) {
        ...
        if (v.IsNumber()) {
            ...
            Log::Warning(Event::Sprite, 
                "Invalid sprite image '%s': value of '%s' must be a number", 
                name, property);
        }
    }
...
bool getBoolean(const JSValue& value, 
                const char* property, 
                const char* name, 
                const bool def = false) 
{
    if (value.HasMember(property)) 
    {
        ...
        if (v.IsBool()) {
            ...
            Log::Warning(Event::Sprite, 
                        "Invalid sprite image '%s': value of '%s' must be a boolean", 
                        name, property);
        }
    }
...
getStretches(const JSValue& value, 
            const char* property, 
            const char* name) 
{
    ...
    if (value.HasMember(property)) 
    {
        ...
        if (v.IsArray()) 
        {
            for (rapidjson::SizeType i = 0; i < v.Size(); ++i) 
            {
                ...
                if (stretch.IsArray() && 
                    stretch.Size() == 2 && 
                    stretch[rapidjson::SizeType(0)].IsNumber() &&
                    stretch[rapidjson::SizeType(1)].IsNumber()) 
                {
                    stretches.emplace_back(
                        style::ImageStretch{stretch[rapidjson::SizeType(0)].GetFloat(),
            ...
            Log::Warning(Event::Sprite, 
                    "Invalid sprite image '%s': value of '%s' must be an array", 
                    name, property);
        }
    }
...
optional<style::ImageContent> 
getContent( const JSValue& value, 
            const char* property, 
            const char* name) 
{
    if (value.HasMember(property)) 
    {
        ...
        if (content.IsArray() && 
            content.Size() == 4 && 
            content[rapidjson::SizeType(0)].IsNumber() &&
            content[rapidjson::SizeType(1)].IsNumber() && 
            content[rapidjson::SizeType(2)].IsNumber() &&
...
std::vector<Immutable<style::Image::Impl>> 
parseSprite(const std::string& encodedImage, 
    ...
    for (const auto& property : properties) 
    {
        ...
        if (value.IsObject()) 
        {
            ...
            auto image = createStyleImage(
                name, raster, x, y, width, height, pixelRatio, sdf, 
                std::move(stretchX), std::move(stretchY), content );
            if (image) {
