...
void BiDi::mergeParagraphLineBreaks(std::set<size_t>& lineBreakPoints) 
{
    ...
    for (int32_t i = 0; i < paragraphCount; i++) 
    {
        ...
        ubidi_getParagraphByIndex(impl->bidiText, i, nullptr, 
                    &paragraphEndIndex, nullptr, &errorCode);

        if (U_FAILURE(errorCode)) {
            throw std::runtime_error(
                std::string("ProcessedBiDiText::mergeParagraphLineBreaks: ") +
...
{
    // BiDi::getLine will error if called across a paragraph boundary, 
    // so we need to ensure that all paragraph boundaries are included
    // in the set of line break points. The calling code might not
    // include the line break because it didn't need to wrap at that 
    // point, or because the text was separated with a more exotic
    // code point such as (U+001C)
...
std::vector<std::u16string> 
BiDi::processText(const std::u16string& input,
...
std::vector<StyledText> 
BiDi::processStyledText(const StyledText& input, 
                        std::set<std::size_t> lineBreakPoints
                        ) 
{
    ...
    ubidi_setPara( impl->bidiText, 
                ...
                &errorCode );    
    if ( U_FAILURE(errorCode) ) {
        throw std::runtime_error(std::string("BiDi::processStyledText: ")
            + u_errorName(errorCode));
    }
    ...
    for (std::size_t lineBreakPoint : lineBreakPoints) 
    {
        ...
        errorCode = U_ZERO_ERROR;
        ubidi_setLine(impl->bidiText, static_cast<int32_t>(lineStartIndex), 
                    static_cast<int32_t>(lineBreakPoint), impl->bidiLine, &errorCode);
        if (U_FAILURE(errorCode)) {
        ...
        for (uint32_t runIndex = 0; runIndex < runCount; runIndex++) 
        {
            ...
            int32_t runLength;
            UBiDiDirection direction = ubidi_getVisualRun(impl->bidiLine, 
                            runIndex, &runLogicalStart, &runLength);
            ...
            if (isReversed) 
            {
                ...
                line.second.insert(line.second.end(), styleIndices.begin() 
                            + logicalStart, 
...
std::u16string BiDi::writeReverse(const std::u16string& input, 
            std::size_t logicalStart, std::size_t logicalEnd) 
{
    UErrorCode errorCode = U_ZERO_ERROR;
    ...
    if (U_FAILURE(errorCode)) {
        throw std::runtime_error(std::string("BiDi::writeReverse: ") 
                            + u_errorName(errorCode));
    }
...
{
    UErrorCode errorCode = U_ZERO_ERROR;
    ubidi_setLine(impl->bidiText, static_cast<int32_t>(start), 
                    static_cast<int32_t>(end), impl->bidiLine, &errorCode);

    if (U_FAILURE(errorCode)) {
        throw std::runtime_error(std::string("BiDi::getLine (setLine): ")
         + u_errorName(errorCode));
    }
