...
template <class T>
GridIndex<T>::GridIndex(const float width_, 
                        const float height_, 
...
template <class T>
std::vector<T> 
GridIndex<T>::query(const BBox& queryBBox) const 
{
    std::vector<T> result;
...
template <class T>
bool GridIndex<T>::hitTest(const BBox& queryBBox, 
                        optional<std::function<bool(const T&)>> predicate
                            ) const 
{
    bool hit = false;
    query(queryBBox, [&](const T& t, const BBox&) -> bool 
...
template <class T>
bool GridIndex<T>::hitTest(const BCircle& queryBCircle, 
                            optional<std::function<bool(const T&)>> predicate
                            ) const 
{
    bool hit = false;
...
{
    return  queryBBox.max.x < 0 || 
            queryBBox.min.x >= width || 
            queryBBox.max.y < 0 || 
...
{
    return  queryBBox.min.x <= 0 && 
            queryBBox.min.y <= 0 && 
            width <= queryBBox.max.x && 
            height <= queryBBox.max.y;
}

template <class T>
typename GridIndex<T>::BBox 
GridIndex<T>::convertToBox(const BCircle& circle) const 
{
...
void GridIndex<T>::query(const BBox& queryBBox, 
                        std::function<bool (const T&, const BBox&)> resultFn
                        ) const 
{
...
template <class T>
bool GridIndex<T>::circlesCollide(const BCircle& first, 
                                  const BCircle& second) const 
{
...
template <class T>
bool GridIndex<T>::circleAndBoxCollide(const BCircle& circle, 
